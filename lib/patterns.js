...
module.exports = function(ctx, text){
  ...
  if(text.match(/bitcoin/i)) cryptoprice.pattern(ctx, 'BTC')
  if(text.match(/ethereum/i)) cryptoprice.pattern(ctx, 'ETH')
  if(text.match(/neo/i)) cryptoprice.pattern(ctx, 'NEO')
  ...
  if(text.match(/\/version/)) ctx.reply('0.2.3')

  if(text.match(/\/cc/)) cryptoprice.code(ctx)
